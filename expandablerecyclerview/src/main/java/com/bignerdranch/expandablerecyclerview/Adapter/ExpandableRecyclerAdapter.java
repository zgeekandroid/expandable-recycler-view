...
public abstract class ExpandableRecyclerAdapter<PVH extends ParentViewHolder, CVH extends ChildViewHolder>
    ...
    public final static int TYPE_HEADER = 3;//头部--支持头部增加一个headerView
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        ...
        if (viewType == TYPE_HEADER) {
            return new HeaderViewHolder(LayoutInflater.from(viewGroup.getContext()).inflate(
                    mHeaderResId, viewGroup, false));
        }
        if (viewType == TYPE_FOOTER) {
            return new FooterViewHolder(
                    LayoutInflater.from(viewGroup.getContext()).inflate(
                            mFooterResId, viewGroup, false));
        }
    ...
    public void setListItem(List<? extends ParentListItem> parentItemList) {
      int old = mParentItemList == null ? 0:  mParentItemList.size();
        int newCount = parentItemList.size() - old;


        mParentItemList =  parentItemList;
        mItemList = ExpandableRecyclerAdapterHelper.generateParentChildItemList(parentItemList);
        /*if (newCount != 0){
            notifyParentItemRangeInserted(old,newCount);
        }*/
        notifyDataSetChanged();
    }
